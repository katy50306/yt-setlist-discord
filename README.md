...
辨識歌單留言
  ...
  （有設優先作者時，直播結束後 N 小時內只看此層）
  優先級 2：時間戳 ≥3 + 讚數 ≥ MIN_LIKES → 多篇合併
  優先級 3：關鍵字匹配 + 時間戳 ≥2 → 加權評分取最高分
    |
    v
...
包含歌單相關關鍵字 + **至少 2 個時間戳**（避免誤抓純感想留言）+ 滿足最低字數/行數門檻的留言，按讚數和文字長度加權評分，取最高分一篇。

### 優先作者冷卻期

設定 `PREFERRED_AUTHORS` 時啟用：直播**結束後** `PREFERRED_AUTHOR_COOLDOWN_HOURS` 小時內（預設 6），只匹配優先作者。冷卻期內沒找到就等下次 cron 重試（state 記錄 `found: false`，24h 內會重試）。

這避免了優先作者還沒貼完整歌單時，被其他人的短感想留言先搶走。冷卻期結束後才會 fallback 到優先級 2/3。

直播尚未結束時（沒有 `actualEndTime`）一律視為冷卻期內。
